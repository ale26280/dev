				...
				{{ Form::open( array('url' => route('ficha_editar_post', $registro['id']), 'method' => 'POST', 'files' => true, 'class' => 'form-horizontal') ) }} 
					...
					<div class="control-group {{ $errors->has( 'imagen_principal' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							<img id="image_imagen_principal" src="{{ URL::to('/') }}/script/timthumb.php?src=/storage/fichas/{{ $registro['imagen_principal'] }}&w=180" /><br/>
					...
					<div class="control-group {{ $errors->has( 'presentacion' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							<a href="/storage/fichas/presentaciones/{{ $registro['presentacion'] }}" title="Ver archivo" target="_blank"><i class="icon-share"></i>Ver PDF Presentacion</a><br/>
					...
					<div class="control-group {{ $errors->has( 'banner' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							@if ( $registro['banner'] != "")
							<img id="image_banner" src="{{ URL::to('/') }}/script/timthumb.php?src=/storage/fichas/{{ $registro['banner'] }}&w=180" /><br/>
							...
							<div class="modal-body" style="height:400px; scroll:auto;">
								...
								@if ( count($fotos) > 0)
									...
									@foreach ($fotos as $foto)
										...
										<img src="/storage/fotos/fichas/{{ $foto['fuente'] }}" width="220" height="auto" /></li>
									@endforeach
					...
					function sendFileToServer(formData,status,tipo,route) {
						...
						var jqXHR=$.ajax({
							...
							success: function(data){
								...
								// Agregamos la nueva imagen al DOM
								var li = $(document.createElement("LI"));
								li.html("<a href='javascript:void%200;' data-element='foto_"+ data.id +"' title='Eliminar' data-delete='/adm/ficha/foto/"+ data.id +"/borrar' class='btn btn-mini deleteModal sec accion'><i class='icon-trash'></i> Eliminar foto</a>");

								li.attr("id", "foto_"+ data.id);

								var img = new Image();
								img.src = '/storage/fotos/fichas/' + data.fuente;
								img.width = 220;
								img.onload = function() {
									li.append($(img));
